...
class CompareJudger:
    ...
    def data(self, subset=None, **kwargs) -> pd.DataFrame:
        if subset is None:
            subset = ["lx", "ly", "lz", "phase", "freeE"]
        ...
        if funcs is not None and callable(funcs):
    ...
    def abs_compare(
        self, phases: Union[list, str], xlabel, ylabel: Union[str, List[str]], **kwargs
    ):
    ...
    def multi_target_ref(self, base, other, xlabel, ylabels, ylabel_name, **kwargs):
        data = self.data
        data = data.sort_values(by=xlabel)
